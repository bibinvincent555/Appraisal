...
# -------------------------------------------
# Time-based Execution Control (9PM–6AM only)
# -------------------------------------------
current_hour=$(date +'%H')
if (( 6 <= 10#$current_hour && 10#$current_hour < 21 )); then
  echo "⏳ Outside allowed window (9PM–6AM). Exiting..."
  exit 0
fi
...
PROCESSED_FILE="$PROCESSED_DIR/${safe_name}_processed.txt"

mkdir -p "$LOG_DIR" "$FAILED_DIR" "$PROCESSED_DIR"
...
for STUDY_UID in "${UNIQUE_UIDS[@]}"; do
  ...
  if grep -Fxq "$STUDY_UID" "$PROCESSED_FILE"; then
    echo "⏩ Skipping already processed UID: $STUDY_UID"
    continue
  fi
